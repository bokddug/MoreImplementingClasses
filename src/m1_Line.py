...
class Line(object):
    ...
    def is_parallel(self, line2):
        ...
        ################################################################
        slopeAD=self.end.x-self.start.x
        slopeBD=line2.end.x-line2.start.x

        if round(slopeAD,12) ==0 or round(slopeBD,12)==0:

            if round(slopeAD, 12) == 0 and round(slopeBD, 12) == 0:
                return True
            return False
        
        slopeA = (self.end.y-self.start.y)//(self.end.x-self.start.x)
        slopeB =(line2.end.y-line2.start.y)//(line2.end.x-line2.start.x)

        if round(slopeA,12)==round(slopeB,12):
            return True
        else:
            return False
